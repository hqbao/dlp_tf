...
for epoch in range(num_of_epoches):
	...
	loss = np.zeros((num_of_train_examples, 5))
	...
	for batch_idx in range(num_of_train_examples):
		...
		for lvl in range(4):
			...
			batch_y.append(tf.expand_dims(input=clzbbe_4dtensor, axis=0))
	...
	with open('{}/{}'.format(output_path, log_file_name), 'a') as log:
		log.write('{}: epoch: {}, train loss: {},  '.format(datetime.now().time(), epoch, np.mean(loss, axis=0)))
	...
	if num_of_validation_examples > 0:
		...
		loss = np.zeros((num_of_validation_examples, 5))
		...
		for batch_idx in range(num_of_validation_examples):
			...
			# get labels
			tboxes, _ = gety(coco=coco, img_id=img_id, classes=classes, frame_mode=frame_mode, mapping=mapping)
