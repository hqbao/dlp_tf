...
def detect(pix, interpreter):
	'''
	'''
	
	x = tf.constant(value=pix, dtype='float32')
	batch_x = tf.expand_dims(input=x, axis=0)
	interpreter.set_tensor(tensor_index=input_details[0]['index'], value=batch_x)
	interpreter.invoke()

	box3d = interpreter.get_tensor(output_details[0]['index'])
	box2d = np.array(box3d[0])
...
clean_after					= 64 # frames
box_pad						= 0 # pixels
entrance					= [200, 200, 270, 400]

interpreter = tf.lite.Interpreter(model_path='{}/rpn_model.tflite'.format(output_path))
...
while True:
	...
	# Execute detection
	box2d = detect(pix=pix, interpreter=interpreter)
	boxes = list(box2d)
	boxes = [box for box in boxes if box[0] != 0 or box[1] != 0 or box[2] != ishape[0] or box[3] != ishape[1]]
