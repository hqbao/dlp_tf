...
for epoch in range(num_of_epoches):
	...
	loss_rpn = np.zeros((num_of_train_examples, 5))
	...
	with open('{}/{}'.format(output_path, log_file_name), 'a') as log:
		log.write('{}: epoch: {}, rpn loss: {}\n'.format(datetime.now().time(), epoch, np.mean(loss_rpn, axis=0)))
		log.write('{}: epoch: {}, det loss: {}\n'.format(datetime.now().time(), epoch, np.mean(loss_det, axis=0)))
	...
	if num_of_validation_examples > 0:
		...
		loss_rpn = np.zeros((num_of_validation_examples, 5))
