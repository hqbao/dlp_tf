...
def randaug(image, bboxes, scale, ishape, mode):
	...
	zoom_image = np.array(zoom_image, dtype='int32')
	zoom_bboxes = []
	for i in range(bbox_len):
		y1, x1, y2, x2, cat = bboxes[i]
...
def augment(image, bboxes, ishape, mode):
	'''
	'''

	area_rank = {}
	for i in range(45**2, 91**2):
		area_rank[i] = 0
	for i in range(91**2, 181**2):
		area_rank[i] = 1
	...
	if t != 0 and t != bbox_len:
		scale = randint(50, 100)/100
		return randaug(image=image, bboxes=bboxes, scale=scale, ishape=ishape, mode=mode)
	...
	if t == 1:
		scale = randint(25, 100)/100
		return randaug(image=image, bboxes=bboxes, scale=scale, ishape=ishape, mode=mode)

def genxy(anno_file_path, image_dir, ishape, abox_2dtensor, iou_thresholds, total_classes, anchor_sampling, mode='train'):
	'''
	Arguments
		anno_file_path:
		image_dir:
	...
	for line_idx in range(total_lines):
		...
		image, bboxes = augment(image=image, bboxes=bboxes, ishape=ishape, mode=mode)

		bbox_2dtensor = tf.constant(value=bboxes, dtype='float32') # (total_bboxes, 4)
		bbox_3dtensor = tf.repeat(input=[bbox_2dtensor], repeats=[abox_2dtensor.shape[0]], axis=0) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, total_bboxes, 4)
		bbox_3dtensor = tf.transpose(a=bbox_3dtensor, perm=[1, 0, 2]) # (total_bboxes, h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 4)

		iou3d = np.zeros((bbox_2dtensor.shape[0], abox_2dtensor.shape[0], 1), dtype='float32') # (total_bboxes, h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 1)
		for i in range(bbox_3dtensor.shape[0]):
			iou_2dtensor = comiou2d(abox_2dtensor=abox_2dtensor, bbox_2dtensor=bbox_3dtensor[i]) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 1)
			iou3d[i] = iou_2dtensor

		iou_3dtensor = tf.constant(value=iou3d, dtype='float32') # (total_bboxes, h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 1)
		iou_2dtensor = tf.squeeze(input=iou_3dtensor, axis=-1) # (total_bboxes, h1*w1*k1 + h2*w2*k2 + h3*w3*k3)
		iou_2dtensor = tf.transpose(a=iou_2dtensor, perm=[1, 0]) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, total_bboxes)
		iou_1dtensor = tf.math.reduce_max(input_tensor=iou_2dtensor, axis=-1) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)
		bbox_indice_1dtensor = tf.math.argmax(input=iou_2dtensor, axis=-1) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)
		...
		bboxes.append([0, 0, 0, 0, total_classes+1])
		matching_2dtensor = tf.gather(params=bboxes, indices=bbox_indice1d) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 5)
		matching_bbox_2dtensor = matching_2dtensor[:, :4] # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 4)
		matching_cat_1dtensor = matching_2dtensor[:, 4] # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)
		clz_2dtensor = tf.one_hot(indices=tf.cast(x=matching_cat_1dtensor, dtype='int64'), depth=total_classes+1, axis=-1) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, total_classes+1)
		loc_2dtensor = comloc2d(bbox_2dtensor=matching_bbox_2dtensor, abox_2dtensor=abox_2dtensor) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 4)
		
		batchy_2dtensor = tf.concat(values=[clz_2dtensor, loc_2dtensor], axis=-1) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, total_classes+1+4)
		batchx_4dtensor = tf.constant(value=[image], dtype='int32')

		yield batchx_4dtensor, batchy_2dtensor, bboxes[:-3], image_id

def genbbox(anno_file_path, image_dir, ishape):
	'''
	'''
	...
	for line_idx in range(total_lines):
		...
		image, bboxes = augment(image=image, bboxes=bboxes, ishape=ishape, mode='test')
