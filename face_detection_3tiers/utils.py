...
def nsm(abox_2dtensor, prediction, nsm_iou_threshold, nsm_score_threshold, nsm_max_output_size, total_classes):
	...
	loc_2dtensor = prediction[:, total_classes+1:] # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 4)
	pbox_2dtensor = loc2box2d(box_2dtensor=abox_2dtensor, bbe_2dtensor=loc_2dtensor) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, 4)

	clz_2dtensor = prediction[:, :total_classes+1] # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3, total_classes+1)
	clz_1dtensor = tf.math.argmax(input=clz_2dtensor, axis=-1) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)

	cancel = tf.where(
		condition=tf.math.less(x=clz_1dtensor, y=total_classes),
		x=1.0,
		y=0.0) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)

	score_1dtensor = tf.math.reduce_max(input_tensor=clz_2dtensor, axis=-1) # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)
	score_1dtensor *= cancel # (h1*w1*k1 + h2*w2*k2 + h3*w3*k3,)
