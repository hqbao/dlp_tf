...
def genheatmap(anno_file_path, img_dir_path, ishape, total_batches, batch_size):
	...
	for batch_idx in range(total_batches):
		...
		heatmap4d = np.zeros((batch_size, ishape[0], ishape[1], 2), dtype='float32')
		...
		for i in range(batch_size):
			...
			image_id = anno[0]
			anno = anno[1:]
			bboxatt_list = [list(map(float, anno[i:i+5])) for i in range(0, len(anno), 5)]
			...
			# Filter
			bboxes = []
			for y1, x1, y2, x2, clz in bboxatt_list:
				...
				bboxes.append([int(y1-crop_y1), int(x1-crop_x1), int(y2-crop_y1), int(x2-crop_x1), int(clz)])

			if len(bboxes) == 0:
				continue

			# Generate heatmap
			heatmap3d = np.zeros((ishape[0], ishape[1], 2), dtype='float32')
			bbox2d = np.array(bboxes)
			for b in range(bbox2d.shape[0]):
				y1, x1, y2, x2, clz = bbox2d[b]
				center_y = y1 + 0.5*(y2 - y1)
				center_x = x1 + 0.5*(x2 - x1)
				a = (y2 - y1)*(x2 - x1)

				pos = np.dstack(np.mgrid[0:ishape[0]:1, 0:ishape[1]:1])
				rv = multivariate_normal(mean=[center_y, center_x], cov=0.07*a, allow_singular=True)
				heatmap2d = rv.pdf(pos)
				heatmap2d /= np.max(heatmap2d)
				heatmap3d[:, :, clz] += heatmap2d
