...
def loss(total_classes, lamda=1.0):
	...
	def ssd_loss(y_true, y_pred):
		...
		loc_loss = tf.math.reduce_sum(input_tensor=balanced_l1(true_loc_2dtensor, pred_loc_2dtensor), axis=-1) # (f,)
...
def build_model(ishape, resnet_settings, k, total_classes):
	...
	tensor = Conv2D(
		...
		kernel_regularizer=regularizers.l2(weight_decay), 
		trainable=trainable, 
		name='conv_prehead')(tensor)
	tensor = BatchNormalization(trainable=bn_trainable, name='conv_prehead_bn')(tensor)
	tensor = Activation('relu')(tensor)
...
def build_test_model(ishape, resnet_settings, k, total_classes, abox_2dtensor, nsm_iou_threshold, nsm_score_threshold, nsm_max_output_size):
	'''
	'''
